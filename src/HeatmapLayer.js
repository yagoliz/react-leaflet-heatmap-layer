...
const LeafletHeatmap = withLeaflet(
  class HeatmapLayer extends MapLayer {
    ...
    fitBounds(): void {
      const { points, leaflet, longitudeExtractor, latitudeExtractor } = this.props;
    ...
    _animateZoom(e: LeafletZoomEvent): void {
      ...
      if (L.DomUtil.setTransform) {
        ...
        this._el.style[L.DomUtil.TRANSFORM] = `${L.DomUtil.getTranslateString(
          offset
        )} scale(${scale})`;
      }
    }
    ...
    reset(): void {
      ...
      if (this._heatmap._width !== size.x) {
        ...
        this._heatmap._width = size.x;
      }
      if (this._heatmap._height !== size.y) {
        this._el.height = size.y;
        this._heatmap._height = size.y;
      }
    ...
    redraw(): void {
      ...
      const getDataForHeatmap = (points, leafletMap, aggregateType) =>
